...
export const preimageSafeMessageHash = (safeAddress: string, message: string, chainId: BigNumberish): string => {
  return ethers.TypedDataEncoder.encode({ verifyingContract: safeAddress, chainId }, EIP712_SAFE_MESSAGE_TYPE, {
    message,
  })
}
...
export const executeTxWithSigners = async (safe: any, tx: SafeTransaction, signers: Signer[], overrides?: any) => {
  const safeAddress = await safe.getAddress()
